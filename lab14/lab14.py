...
def prune_min(t):
    ...
    for branch in t.branches:
        if branch.label < smallest.label:
            smallest = branch
    prune_min(smallest)
    t.branches = [Tree(smallest.label, smallest.branches)]
...
class CheckingAccount(Account):
    ...
    "*** YOUR CODE HERE ***"
    def deposit_check(self, check_object):
        if self.holder == check_object.payable and not check_object.deposited:
            self.deposit(check_object.amount)
            check_object.deposited = True
            return check_object.amount
        else:
...
def filterl(lst, pred):
    ...
    if lst is Link.empty:
        return lst
    return foldl(foldl(lst, lambda x, y: Link(y, x) if pred(y) else x, Link.empty), lambda x, y: Link(y, x), Link.empty)
...
def contains(s, x):
    return (s is not Link.empty) and (True if x == s.first else contains(s.rest, x))
