...
def swine_align(player_score, opponent_score):
    ...
    if player_score > opponent_score:
        factor = player_score - 1
    elif opponent_score > player_score:
        factor = opponent_score - 1
    elif player_score == opponent_score and player_score >= 10:
        return True
    else:
        return False
    while factor > 0:
        if player_score % factor == 0 and opponent_score % factor == 0:
            break
        factor -=1
    if factor >= 10:
        return True
    ...
    def say_highest(score0, score1):
        if who == 0:
            if score0 - last_score > running_high:                
                print("{} point(s)! The most yet for Player 0".format(score0 - last_score))
            ...
            if score1 - last_score > running_high:
                print("{} point(s)! The most yet for Player 1".format(score1 - last_score))
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average_return(*args):
        count_times = 0
        total = 0
        while count_times < trials_count:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while number_of_dice <= 10:
        ...
        score = average_score(number_of_dice, dice)
        if score > highest_average_score:
            highest_average_score = score
            highest_dice = number_of_dice
        number_of_dice += 1
    return highest_dice
    # END PROBLEM 9
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score > opponent_score:
        if score + free_bacon(opponent_score) >= 100:
            return 0
        number_of_dice = 1
        while number_of_dice <= 6:
            average_score = make_averaged(roll_dice)
            predict_score = average_score(number_of_dice)
            if (score + predict_score) >= 100.0:
