...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        while not player.deck.is_empty():
            kard = player.deck.draw()
            kard.attack = kard.attack + other_card.attack
            kard.defense = kard.defense + other_card.defense
            player_copy.append(kard)            
        player.deck = Deck(player_copy)

        opponent_copy = []
        while not opponent.deck.is_empty():
            kard = opponent.deck.draw()
            if kard.attack != other_card.attack and kard.defense != other_card.defense:
                opponent_copy.append(kard)
        opponent.deck = Deck(opponent_copy)
