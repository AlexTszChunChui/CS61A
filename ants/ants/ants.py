...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while currentplace != beehive:
            if currentplace.bees and self.min_range <= location <= self.max_range:
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        # To see if it is true queen and remove imposter
        ...
        # Adding damage to all ant behind Queenant
        currentplace = self.place
        while currentplace.exit is not None:
            if currentplace.exit.ant is not None and not currentplace.exit.ant.doubled:
                currentplace.exit.ant.damage = currentplace.exit.ant.damage * 2
                currentplace.exit.ant.doubled = True
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.true_queen and amount >= self.armor and type(self.place) != Water:
            bees_win()
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        if self.contained_ant is None and isinstance(other, ContainerAnt) is False:
            return True
        return False
        # END Problem Optional 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while current is not beehive:
            for bee in current.bees:
                in_front[bee] = distance
            if current.ant is not None and current.ant != self:
                in_front[current.ant] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        return self.damage - (self.insects_shot * 0.05 + distance * 0.2)
        # END Problem Optional 5
