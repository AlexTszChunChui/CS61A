...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        expr_operands = expr.rest.map(lambda x: scheme_eval(x, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"        
        body = do_lambda_form(Pair(target.rest, expressions.rest), env)
...
def do_and_form(expressions, env):
    ...
    while expressions!= nil:
        revalue = scheme_eval(expressions.first, env)
        if is_false_primitive(revalue):
            return False
        elif expressions.rest == nil and is_true_primitive(revalue):
            return revalue
        expressions = expressions.rest
    return True
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        validate_form(cond, 1)
        if cond.first == 'else':
            test = True
            if expressions.rest != nil:
                raise SchemeError('else must be last')
        else:
            test = scheme_eval(cond.first, env)
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            if cond.rest != nil:
                return eval_all(cond.rest, env)
            return test
            # END PROBLEM 13
        expressions = expressions.rest
    return None
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    while bindings != nil:
        binds = bindings.first
        validate_form(binds, 2, 2)
        names = Pair(binds.first, names)
        values = Pair(scheme_eval(binds.rest.first, env), values)
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            if scheme_symbolp(first) and first in SPECIAL_FORMS:
                ...
                expr_operator = scheme_eval(first, env)
                validate_procedure(expr_operator)
                expr_operands = rest.map(lambda x: scheme_eval(x, env))
                result = scheme_apply(expr_operator, expr_operands, env)
