...
def totals_tree(m):
    ...
    result_branch = []
    if is_planet(m):
        return tree(total_weight(m), result_branch)

    result_branch += [total_weight(end(left(m))), total_weight(end(right(m)))]
...
def has_path(t, word):
    ...
    if len(word) == 1 and label(t) == word:
        return True
    elif (is_leaf(t) and len(word) != 1) or label(t) != word[0]:
        return False
    for branch in branches(t):
